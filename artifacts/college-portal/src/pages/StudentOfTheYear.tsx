...
export default function StudentOfTheYear() {
  ...
  return (
    ...
    >
      {/* Banner */}
      <div className="relative rounded-3xl overflow-hidden bg-card border border-border p-10 mb-12 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="relative z-10 flex gap-2 bg-muted/60 p-2 rounded-2xl backdrop-blur-md border border-border">
          {(["2026", "2025", "2024", "2023"] as const).map(year => (
            <button
              key={year}
              onClick={() => setSelectedYear(year)}
              data-testid={`year-selector-${year}`}
      ...
      >
        <div className="lg:col-span-1 rounded-3xl bg-card border border-border p-8 flex flex-col items-center text-center relative overflow-hidden group">
          ...
          <div className="relative w-40 h-40 rounded-full mb-6 border-4 border-background bg-gradient-to-br from-blue-400 to-purple-500 p-1 shadow-2xl">
            <div className="w-full h-full rounded-full bg-card flex items-center justify-center">
              <span className="text-5xl font-bold text-foreground">{student.name.charAt(0)}</span>
            </div>
          ...
          <p className="text-blue-500 font-medium mb-6 text-lg">{student.branch}</p>

          <div className="w-full mt-auto pt-6 border-t border-border">
            <div className="flex justify-between items-center mb-2">
              <span className="text-muted-foreground">Achievement Score</span>
              <span className="font-bold text-foreground">{student.achievementScore}/100</span>
            </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 gap-4">
          <div className="p-6 rounded-3xl bg-card border border-border flex flex-col justify-center">
            ...
            <div className="text-4xl font-bold text-foreground mb-2">{student.cgpa}</div>
          ...
          <div className="p-6 rounded-3xl bg-card border border-border flex flex-col justify-center">
            ...
            <div className="text-4xl font-bold text-foreground mb-2">#{student.codingRank}</div>
          ...
          <div className="p-6 rounded-3xl bg-card border border-border flex flex-col justify-center">
            ...
            <div className="text-4xl font-bold text-foreground mb-2">{student.competitionWins}</div>
          ...
          <div className="p-6 rounded-3xl bg-card border border-border flex flex-col justify-center">
            ...
            <div className="text-4xl font-bold text-foreground mb-2">{student.researchContributions}</div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" data-testid="dropdown-branch-filter">
              ...
              <tbody>
                {filteredLeaderboard.map((user, idx) => (
                  <motion.tr
                    ...
                    key={user.rank}
                    data-testid={`row-candidate-${user.rank}`}
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-bold ${user.rank <= 3 ? 'text-foreground' : 'text-muted-foreground'}`}>
                          #{user.rank}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="p-4 font-bold text-blue-500">{user.totalPoints.toLocaleString()}</td>
                    <td className="p-4 text-foreground">{user.codingScore.toLocaleString()}</td>
                    <td className="p-4 text-foreground">{user.mathScore.toLocaleString()}</td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
